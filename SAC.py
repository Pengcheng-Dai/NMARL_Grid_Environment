...
class SACOptimizer:
    def __init__(self, grid_size, network, agent_list, state_num, action_num, init_states, goal_states, gamma, horizon,
        ...
        # grid_size, num_agents, goal, adjacency
        ...
        for t in range(self.horizon - 1):
            for i in range(self.agent_num):
                ...
                local_Q_value[local_state_t_code, local_action_t_code] = (1 - rate_w) * local_Q_value[
                    local_state_t_code, local_action_t_code] + rate_w * (self.game_simulator.global_reward_history[t][
                                                                             i] + self.gamma * local_Q_value[
...
#     np.random.seed(t_seed)
    ...
    for m in trange(T):
        ...
        # 保存关键数据
        np.save("./multi_agent/objective_perform_SAC_more{}.npy".format(t_seed), SAC_objective_perform)
