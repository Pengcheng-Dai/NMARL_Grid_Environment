...
class EDROptimizer:
    def __init__(self, grid_size, network, agent_list, state_num, action_num, init_states, goal_states, gamma, horizon,
        ...
        self.reward_list = self.construct_obervation_table(hop=1) # 奖励的定义
...
#     np.random.seed(t_seed)
    ...
    for m in trange(T):
        ...
        # 保存关键数据
        np.save("./multi_agent/objective_perform_EDR_1_1_{}.npy".format(t_seed), EDR_objective_perform)
        # np.save("./multi_agent_crate/aver_gradient_EDR_maxmore{}.npy".format(t_seed), EDR_aver_gradient)
